...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_params):
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    a2, norm_cache = batchnorm_forward(a, gamma, beta, bn_params)
...
def affine_norm_relu_backward(dout, cache):
    ...
    da2, dgamma, dbeta = batchnorm_backward_alt(da, norm_cache)

    dx, dw, db = affine_backward(da2, fc_cache)
    return dx, dw, db, dgamma, dbeta

def affine_laynorm_relu_forward(x, w, b, gamma, beta, ln_params):
    """
    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    a2, norm_cache = layernorm_forward(a, gamma, beta, ln_params)
    out, relu_cache = relu_forward(a2)
    cache = (fc_cache, norm_cache, relu_cache)
    return out, cache

def affine_laynorm_relu_backward(dout, cache):
    fc_cache, norm_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)

    da2, dgamma, dbeta = layernorm_backward(da, norm_cache)
